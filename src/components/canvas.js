...
class CanvasComponent extends React.Component {
    componentDidMount() {
        ...
        window.onload = () => {
            ...
            canvas.addEventListener("click", event => {
                if (
                    this.props.game.player.isAlive === false ||
                    this.props.game.won
                ) {
    ...
    draw(init) {
        ...
        if (playerPos.x) {
            vision(
                playerPos.x,
                playerPos.y,
                this.props.game.player.vision
            ).forEach(position => {
                if (
                    typeof this.props.maps[this.props.game.level - 1][
                        position.y
                    ][position.x] === "number"
                ) {
                    fillBackground(
                        this.props.maps[this.props.game.level - 1][position.y][
                            position.x
                        ],
                        c,
                        position.y,
                ...
                fillObjects(
                    this.props.game.gameState[position.y][position.x].name,
                    c,
                    position.y,
                    position.x
                );
    ...
    /* Render */
    render() {
        return <canvas ref="canvas" tabIndex="1"></canvas>;
    }
}
...
function Population(
    ...
    this.skeleton = Object.assign({}, skeleton, {
        ...
        number:
            skeleton.number + (Math.floor(Math.random() * 3) - 1) + level * 2
    });
...
function placeObjects(gameState, population) {
    ...
    for (var object in population) {
        for (var i = 0; i < population[object].number; i += 1) {
            ...
            gameState[emptyPlace[num].y][emptyPlace[num].x] = Object.assign(
                {},
                population[object]
            );
...
function vision(x, y, vision) {
    ...
    for (var i = -vision; i < vision + 1; i += 1) {
        for (
            var k = -vision + Math.abs(i);
            k < vision - Math.abs(i) + 1;
            k += 1
        ) {
